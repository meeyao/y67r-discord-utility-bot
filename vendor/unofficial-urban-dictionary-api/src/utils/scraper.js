...
function extractContributorDate($, el) {
	const contributor = $(el).find(".contributor").first();
	if (!contributor.length) return "";
	...
	const bylinePrefix = $(el).find(".contributor a").first().text().trim();
	if (!bylinePrefix) return text;

	return text.replace(bylinePrefix, "").replace(/\s+/g, " ").trim();
}
...
function extractDetails($, el) {
	return {
		...
		date: extractContributorDate($, el),
	};
}
